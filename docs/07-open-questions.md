...
| 4 | ~~Is location filtering done via Searchstax facets or Drupal JSON:API filters?~~ | Backend team | **Resolved** — Location-only filtering (no keyword) uses Drupal JSON:API filter on `brickParkLocation.id`. Combined keyword + location filtering routes through Searchstax using `fq=ss_zone_uuid:({uuid1} OR {uuid2})`. |
...
| 13 | ~~The Searchstax `ss_uuid` hydration requires one Drupal call per brick (`/brick/{uuid}`). For a page of 20 results, that's 20 parallel requests. Is there a batch endpoint?~~ | Backend team | **Resolved** — Yes, batch endpoint available: `/bricks?filter[id][operator]=IN&filter[id][value]={uuid1,uuid2,...}`. However, Phase 3 now uses **zero-hydration** (`ss_file_img_uuid` added to index), so batch hydration is not needed for the primary keyword path. It remains available for the Drupal CONTAINS fallback if SearchStax is down. |
