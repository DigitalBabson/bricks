...
| 3 | ~~Does the Drupal bricks endpoint return a total count?~~ | Backend team | **Resolved** — No. No `meta.count`. Derive pagination from `links.next` presence. For Searchstax, `numFound` provides the total. |
